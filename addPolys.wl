...
MultP[v1_,v2_]:=DropExtra[Map[Function[x,SimplifyFP[x]],vector=Table[0,{i,Length[v1]+Length[v2]-1}];
For[j=1,j<=Length[v2],j++,For[i=1,i<=Length[v1],i++,vector[[j+i-1]]=vector[[j+i-1]]+v1[[i]]*ThetanFP[v2[[j]],i-1]]];vector]]
